...
| **F5_FRAGILITY** | Маяк унаследовал D_NEEDS_HEALTHCHECK — хрупкость была изначально. S_DISCOVERY её усиливает, S_HEALTHCHECK (Сердцебиение) — единственный способ закрыть |
...
- Долг `D_NEEDS_HEALTHCHECK` → **F5_FRAGILITY** — старый маяк никогда не имел проверки здоровья узлов. Узлы могут быть мёртвыми, но маяк об этом не знает. Это стартовая хрупкость, которую необходимо устранить.
...
**Наследованная хрупкость.** Маяк уже сломан — D_NEEDS_HEALTHCHECK пришёл с историей. S_HEALTHCHECK обязателен в любом пути: он и закрывает долг, и даёт HAS_FAILOVER.

**Два пути к маршрутизации:**
- **Путь A** (простой): только S_TRAFFIC_SPLIT. Никакого конфликта. Дешевле, но одна стратегия роутинга.
- **Путь B** (продвинутый): S_TRAFFIC_SPLIT + S_HASH_ROUTING + S_POLICY_RESOLVER + R_PATRON_SEAL. Обе стратегии сосуществуют, но конфликтуют без Арбитра, и R_PATRON_SEAL расширяет лимит ауры.

**Главная ловушка:** положить оба роутинга без арбитра → F2_CONTRADICTION.
**Вторая ловушка:** проигнорировать S_HEALTHCHECK → F5_FRAGILITY (унаследована с самого старта).
